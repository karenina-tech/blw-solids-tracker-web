...
export function FoodCatalog() {
  ...
  return (
    <div>
      <div className="mb-4">
        <h2 className="text-xl font-bold text-slate-800 mb-1">{t('catalog.title')}</h2>
        <p className="text-slate-500 text-sm">{t('catalog.subtitle')}</p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-xs text-slate-400">
          ...
          {totalPages > 1 ? ` · ${t('catalog.page', { current: page, total: totalPages })}` : ''}
        </span>
      </div>
                ...
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="w-10 px-3 py-2.5 text-center text-xs font-semibold text-slate-400 uppercase tracking-wide rounded-tr-xl">
                    <div className="relative group inline-flex justify-center">
                      <span>⚠️</span>
                      <Tooltip text={t('catalog.columns.hazard')} width="w-32" />
                    </div>
                  </th>
                </tr>
              </thead>
                      ...
                      {isExpanded && (
                        <tr className="bg-white border-b border-slate-200">
                          <td colSpan={4} className="px-4 pb-4 pt-0">
                            <div className="space-y-3 pt-3 border-t border-accent-300 mt-1">
                              {Object.entries(food.preparationByAge).map(([range, instruction]) => (
                                <div key={range}>
                                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                                    {t('catalog.months', { range })}
                                  </span>
                                  <p className="text-sm text-slate-600 mt-0.5 leading-relaxed">{instruction as string}</p>
                                </div>
                              ))}
                              {food.chokingHazardWarning && (
