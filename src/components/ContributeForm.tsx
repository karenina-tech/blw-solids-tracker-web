...
export function ContributeForm() {
  ...
  const validate = (): boolean => {
    ...
    if (!fields.dietaryType) errs.dietaryType = t('contribute.form.required_selection', { field: 'dietary type' });
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2 sm:col-span-1">
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                {t('contribute.form.nameLabel')} <span className="text-red-400">{t('contribute.form.required')}</span>
              </label>
              <input
                type="text"
                value={fields.name}
              ...
              />
              {duplicateFood
                ? <p className="text-xs text-amber-600 mt-1">{t('contribute.form.duplicate', { name: duplicateFood.name })}</p>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              {t('contribute.form.categoryLabel')} <span className="text-red-400">{t('contribute.form.required')}</span>
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              {t('contribute.form.dietaryTypeLabel')} <span className="text-red-400">{t('contribute.form.required')}</span>
            </label>
            <div ref={dietRef} className="relative">
              ...
              >
                {fields.dietaryType ? (
                  ...
                  <span className="text-slate-400">{t('contribute.form.dietaryTypeOptions.placeholder')}</span>
                )}
                <svg className={`w-4 h-4 shrink-0 transition-transform ${dietOpen ? 'rotate-180 text-accent-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="absolute z-50 top-full mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg py-1 select-none">
                  {DIETARY_OPTIONS.map(({ value, emoji }) => (
                    <button
                      key={value}
                      type="button"
                      onClick={() => { set('dietaryType', value); setDietOpen(false); }}
                    ...
                    >
                      ...
                      {fields.dietaryType === value && <span className="ml-auto text-accent-600 text-xs">✓</span>}
                    </button>
                  ))}
                </div>
              )}
          ...
          {(['prep6_9', 'prep9_12'] as const).map((field) => {
            const labelKey = field === 'prep6_9' ? 'contribute.form.prep6_9Label' : 'contribute.form.prep9_12Label';
            const placeholderKey = field === 'prep6_9' ? 'contribute.form.prep6_9Placeholder' : 'contribute.form.prep9_12Placeholder';
            ...
            return (
              <div key={field}>
                ...
                {!errors[field] && len > 0 && len < PREP_MIN
                  ? <p className="text-xs text-amber-500 mt-1">{t('contribute.form.minProgress', { min: PREP_MIN, remaining: PREP_MIN - len })}</p>
          ...
          <div>
            <div className="flex justify-between items-baseline mb-1.5">
              <label className="block text-sm font-medium text-slate-700">
                ...
                <span className="text-xs font-normal text-slate-400">{t('contribute.form.chokingOptional')}</span>
              </label>
          ...
          <div className="flex items-center justify-end pt-1">
            ...
            >
              {submission.status === 'loading' ? t('contribute.form.submitting') : t('contribute.form.submit')}
            </button>
          </div>
        </form>
      )}
