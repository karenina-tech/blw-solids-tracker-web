...
const OPTIONS: { value: DietType; labelKey: string; descriptionKey: string }[] = [
  {
    value: 'standard',
    labelKey: 'steps.dietType.standard.label',
    descriptionKey: 'steps.dietType.standard.description',
  },
  {
    value: 'vegetarian',
    labelKey: 'steps.dietType.vegetarian.label',
    descriptionKey: 'steps.dietType.vegetarian.description',
  },
  {
    value: 'vegan',
    labelKey: 'steps.dietType.vegan.label',
    descriptionKey: 'steps.dietType.vegan.description',
  },
];
...
export function StepDietType({ dietType, onChange, onNext, onBack }: StepDietTypeProps) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-slate-800 mb-1">{t('steps.dietType.title')}</h2>
      <p className="text-slate-500 text-sm mb-6">{t('steps.dietType.subtitle')}</p>
          ...
          >
            <div
              className={`mt-0.5 w-5 h-5 rounded-full border-2 shrink-0 flex items-center justify-center ${
                dietType === opt.value ? 'border-accent-600' : 'border-slate-300'
              }`}
            >
              {dietType === opt.value && (
