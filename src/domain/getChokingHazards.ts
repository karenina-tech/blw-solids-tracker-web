...
export function getChokingHazardsTool(repo: IFoodRepository, input: GetChokingHazardsInput) {
  ...
  const preparationRules: PreparationRule[] = repo
    .getAll()
    .filter((food) => !!food.chokingHazardWarning)
    .map((food) => ({
      id: food.id,
      name: food.name,
